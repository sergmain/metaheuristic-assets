...
| mh-rg-requirements-from-batch-1.0 | rg-requirements | Reads every file named by one meta-storage batch record, has CC derive requirements from each file in parallel (one mh.batch-line-splitter branch per file), stores them all in a fresh RG project as one chain of snapshots, then deletes the record. Uses rg-temp-project and call-cc. |
